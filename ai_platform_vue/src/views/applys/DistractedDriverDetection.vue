  ...
  <div>
    <div class="apply_description">
      <div class="apply_description_container">
        <div class="apply_description_name">驾驶员状态检测</div>
        <div class="apply_description_detail">检测驾驶员的驾驶状态，目前可检测9种状态</div>
      </div>
    </div>
    <el-upload
      class="upload-demo"
      action="/api/apply/distracted_driver_detection"
  ...
  methods: {
    ...
    onChange(file){
      ...
      reader.onload  = function(event){
        ...
        _this.$axios.post("/api/apply/distracted_driver_detection",{base64Data:img_base64}).then(res=>{
          let data=res.data;
          if(data.code===200){
            //检测成功
            _this.loading=false;
            _this.url="data:image/jpeg;base64,"+data.data.base64Data;
            _this.confidences='(概率：'+data.data.confidences+'%)';
            _this.status='驾驶员正在'+data.data.status;
          }else{
            //检测失败
            _this.loading=false;
            _this.isShow=false;
    ...
    takePhoto(){
      ...
      this.$axios.post('/api/apply/distracted_driver_detection',{base64Data:base64Data}).then(res=>{
        this.stop=false; //开始下一次拍摄上传
        this.confidences='(概率：'+res.data.data.confidences+'%)';
        this.status='驾驶员正在'+res.data.data.status;
